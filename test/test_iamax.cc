...
void test_iamax_work( Params& params, bool run, Policy policy = blam_default_policy )
{
    namespace blas = blam;
    ...
    // test error exits
    int64_t result;
    assert_throw( blas::iamax( policy, -1, x, incx, result ), blas::Error );
    assert_throw( blas::iamax( policy,  n, x,    0, result ), blas::Error );
    assert_throw( blas::iamax( policy,  n, x,   -1, result ), blas::Error );
    ...
    double time;
    {
      ...
      blas::iamax( policy, n, dx, incx, result );
      time = timer.seconds();
