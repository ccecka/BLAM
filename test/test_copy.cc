...
void test_copy_work( Params& params, bool run, Policy policy = blam_default_policy )
{
    namespace blas = blam;
    ...
    // test error exits
    assert_throw( blas::copy( policy, -1, x, incx, y, incy ), blas::Error );
    assert_throw( blas::copy( policy,  n, x,    0, y, incy ), blas::Error );
    assert_throw( blas::copy( policy,  n, x, incx, y,    0 ), blas::Error );
    ...
    double time;
    {
      ...
      blas::copy( policy, n, dx, incx, dy, incy );
      time = timer.seconds();
