...
void test_scal_work( Params& params, bool run, Policy policy = blam_default_policy )
{
    namespace blas = blam;
    ...
    // test error exits
    assert_throw( blas::scal( policy, -1, alpha, x, incx ), blas::Error );
    assert_throw( blas::scal( policy,  n, alpha, x,    0 ), blas::Error );
    assert_throw( blas::scal( policy,  n, alpha, x,   -1 ), blas::Error );
    ...
    double time;
    {
      ...
      blas::scal( policy, n, alpha, dx, incx );
      time = timer.seconds();
