...
void test_asum_work( Params& params, bool run, Policy policy = blam_default_policy )
{
    namespace blas = blam;
    ...
    // test error exits
    real_t result;
    assert_throw( blas::asum( policy, -1, x, incx, result ), blas::Error );
    assert_throw( blas::asum( policy,  n, x,    0, result ), blas::Error );
    assert_throw( blas::asum( policy,  n, x,   -1, result ), blas::Error );
    ...
    double time;
    {
      ...
      blas::asum( policy, n, dx, incx, result );
      time = timer.seconds();
