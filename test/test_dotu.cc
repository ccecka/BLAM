...
void test_dotu_work( Params& params, bool run, Policy policy = blam_default_policy )
{
    namespace blas = blam;
    ...
    // test error exits
    scalar_t result;
    assert_throw( blas::dotu( policy, -1, x, incx, y, incy, result ), blas::Error );
    assert_throw( blas::dotu( policy,  n, x,    0, y, incy, result ), blas::Error );
    assert_throw( blas::dotu( policy,  n, x, incx, y,    0, result ), blas::Error );
    ...
    double time;
    {
      ...
      blas::dotu( policy, n, dx, incx, dy, incy, result );
      time = timer.seconds();
